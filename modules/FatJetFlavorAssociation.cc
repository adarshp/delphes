...
void FatJetFlavorAssociation::Process(){
  ...
  TObjArray *partonLHEFArray = 0;
    ...
    Candidate *element;
    int i=1;
    cout << "####################Before Filtered, Partons####################" << endl;
    while((element = static_cast<Candidate *>(fItPartonInputArray->Next())) != 0)
    {
        cout << i << " " << element->PID << " " << element->Status << " " << element->Momentum.Pt() << " " << element->Momentum.Eta() << endl;
        i++;
    }
        cout << "####################Before Filtered, Partons####################" << endl;}
    //test*/
  ...
  partonArray = fPartonFilter->GetSubArray(fPartonClassifierFat, 0); // get the filtered parton array
    ...
    /*//test
    {TIterator *fItParton;
    fItParton = partonArray->MakeIterator();
    Candidate *element;
    int i=1;
    cout << "####################After Filtered, Partons####################" << endl;
    while((element = static_cast<Candidate *>(fItParton->Next())) != 0)
    {
        cout << i << " " << element->PID << " " << element->Status << " " << element->Momentum.Pt() << " " << element->Momentum.Eta() << endl;
        i++;
    }
  ...
  if(partonArray == 0) return;
    ...
    /*//test
    int test=0;
    cout << "Before LHEF Input, test=" << test << endl;
    //test*/
  ...
  if(fParticleLHEFInputArray)
  {
    ...
    partonLHEFArray = fParticleLHEFFilter->GetSubArray(fParticleLHEFClassifierFat, 0); // get the filtered parton array
      ...
      /*//test
      test=1;
      cout << "In LHEF Input, test=" << "test" << endl;
      //test*/
  }
    
    /*//test
    test=2;
    cout << "After LHEF Input, test=" << test << endl;
    //test*/
  ...
  fItJetInputArray->Reset();
    ...
    int i=1;
    cout << "####################Jets####################" << endl;
    while((element = static_cast<Candidate *>(fItJetInputArray->Next())) != 0)
    {
        cout << i << " " << element->PID << " " << element->Status << " " << element->Momentum.Pt() << " " << element->Momentum.Eta() << endl;
        i++;
    }
  ...
  while((jet = static_cast<Candidate *>(fItJetInputArray->Next())))
  {
    ...
    GetAlgoFlavor(jet, partonArray, partonLHEFArray);
      if(fParticleLHEFInputArray) {GetPhysicsFlavor(jet, partonArray, partonLHEFArray); cout << "used" << endl;}
  }
...
void FatJetFlavorAssociation::GetAlgoFlavor(Candidate *jet, TObjArray *partonArray, TObjArray *partonLHEFArray)
{
  ...
  while((parton = static_cast<Candidate *>(itPartonArray.Next())))
  {
    ...
    if(!fParticleLHEFInputArray) continue;
      ...
      //test
      cout << "ParticleLHEFInputArray exist" << endl;
      //test
